...
- 전체 transaction(`BEGIN`/`COMMIT`)이며, 첫 번째 `DO` 블록이 admission-year 범위 미확정을 감지해 즉시 예외를 발생시킨다.
- 현재 import의 `admissionYearFrom=null`을 schema의 `curriculum_versions.admission_year_from NOT NULL`에 억지로 매핑하지 않았다. 따라서 현재 파일을 실행하면 어떤 row도 남지 않고 rollback된다.
- blocker 해소 후에는 실제 `schools(school_id,name)` unique 조건을 이용해 전자공학과를 조건부 생성하고, draft version을 확인·재사용한다.
...
6. `admission_year_from`의 공식 적용 범위를 확정하기 전에는 현재 fail-closed blocker를 제거하지 않음
...
- 현재 SQL은 첫 blocker에서 rollback되므로 cleanup DML이 필요하지 않다.
- 향후 blocker 해소 후에도 오류가 발생하면 transaction 전체를 rollback하며, 기존 row를 임의로 삭제하거나 덮어쓰지 않는다.
...
1-D-5B 실제 적용 검토는 **보류**한다. admission-year 범위와 전자공학과 department row가 확정되면 재검토할 수 있다. 이번 단계에서는 seed SQL을 실행하지 않았다.
