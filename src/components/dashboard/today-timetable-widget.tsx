  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link href="/mypage" className="text-sm font-semibold text-emerald-600 hover:text-emerald-700">
          마이페이지에서 관리
        </Link>
      </div>
      ...
      {todayCourses.length === 0 ? (
        <div className="text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-200">
          ...
          <p className="text-[11px] text-gray-400 mt-2">마이페이지에서 시간표를 등록하거나 수정해 보세요.</p>
        </div>
      ) : (
          ...
          {todayCourses.map((course) => (
            <div key={course.id} className="rounded-2xl border border-gray-100 p-4 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-gray-900">{course.course.name}</p>
